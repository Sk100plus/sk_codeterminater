// You are given an array find number of different 
// partition that can be made so that absolute difference
// of the sum of elements equal to D;
...
int main(){
    ...
    cin>>d;
    int totalS=0;
    for(int i=0;i<n;i++)totalS+=vec[i];
int target=(totalS-d)/2;
if((totalS-d)<0||(totalS-d)%2!=0){
    cout<<"NO ANY SUBSET OCCUR"<<endl;
}
else
    cout<<f(vec,target,0,n);
    return 0;
}